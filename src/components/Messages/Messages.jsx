...
const Messages = (props) => {
  let MessagesElements = props.MessagesData.map((messages) => (
    <Dialog messages={messages.message} key={messages.id} id={messages.id} />
  ));

  let DialogElements = props.DialogsData.map((dialog) => (
    <DialogName name={dialog.name} key={dialog.id} id={dialog.id} />
  ));
    ...
    <div className={messages.messages}>
      ...
      <div className={messages.dialog}>
        <div className={messages.inner}>
          <div className={messages.content}>{MessagesElements}</div>
